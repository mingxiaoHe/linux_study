...
def get_method_pager(page, all_page_count, murl):
    ...
    for i in range(all_page_count):
        if page == i+1:  # i = 0 时，第一页
            present_html = '<li class="active"><a href="%s%d">%d</a></li>' % (murl, i+1, i+1 )
        else: # 第二页到尾页
            present_html = '<li class=""><a href="%s%d">%d</a></li>' % (murl, i+1, i+1 )
        page_html.append(present_html)
